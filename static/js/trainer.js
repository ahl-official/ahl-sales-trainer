        ...
        async function loadOnboardingStatus() {
            try {
                const resp = await fetch(`${API_BASE}/api/training/progress`, { credentials: 'include' });
                if (!resp.ok) return;
        ...
        async function getDeepgramToken() {
            try {
                const res = await fetch(`${API_BASE}/api/deepgram-token`, { credentials: 'include' });
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', async () => {
                ...
                try {
                    const resp = await fetch(`${API_BASE}/api/sessions/${sessionState.sessionId}/export/pdf`, { method: 'GET', credentials: 'include' });
                    if (!resp.ok) {
        ...
        async function checkForResume() {
            try {
                const resp = await fetch(`${API_BASE}/api/training/resume-check`, { credentials: 'include' });
                if (!resp.ok) return;
                const data = await resp.json();
